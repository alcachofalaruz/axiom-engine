...
parse_odin_file :: proc(schema: ^Meta_Schema, path: string) -> bool {
	...
	if schema.package_name == "" {
		schema.package_name, alloc_err = strings.intern_get(&schema.names, file.pkg_name)
		if alloc_err != nil {
			return false
		}
	} else if file.pkg_name != schema.package_name {
		fmt.eprintfln(
			"Conflicting packages: expected {}, found {} in {}",
			schema.package_name,
			file.pkg_name,
			path,
		)
...
begin_generated_file :: proc(package_name: string, builder: ^strings.Builder) {
	fmt.sbprintln(builder, "package ", package_name)
	fmt.sbprintln(builder)
...
emit_schema_files :: proc(schema: ^Meta_Schema, source_directory: string) -> bool {
	...
	for declaration in schema.fixed {
		...
		if !write_fixed_file(source_directory, file_name, schema.package_name, declaration) {
			return false
		}
	}
	for declaration in schema.vectors {
		...
		if !write_vector_file(source_directory, file_name, schema.package_name, declaration) {
			return false
		}
	}
